                ...
                <el-form-item :label="item.formColumnName" :prop="item.prop" :key="index" :rules="item.formValidate">
                    ...
                    <template v-else-if="item.formColumnType === 'p-dict'">
                        <el-select v-model="form[item.prop]"
                                   @change="$emit('form-item-change', {key: item.prop, value: form[item.prop]})" style="width: 100%;">
                            <el-option
    ...
    handleSave () {
      this.$refs.dialogForm.validate((valid) => {
        if (valid) {
          let request
          if (this.editType === 0) {
            request = this.$api.datasource.create({ ...this.form })
          } else {
            request = this.$api.datasource.update({ ...this.form })
          }
          request
            .then(res => {
