<template>
  <div>
    ...
    >
      ...
      <ele-table-column v-for="(item, index) in activeColumns"
                       :key="index"
      ...
      <ele-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button type="text" @click="handleDetailDialogOpen(scope.row)" v-if="auth.detail">合集详情</el-button>
        </template>
      </ele-table-column>
    </ele-table>
    ...
    <union-detail-dialog ref="unionDetailDialog"
                 :formData="dialogForm"
                 :title="dialogTitle"
                 :editType="editType"
                 :bizConfig="bizConfig"
...
export default {
  ...
  components: {
    ...
    UnionDetailDialog: () => import('./PageDialog/union-detail'),
    eleTable,
    eleTableColumn 
  },
  data () {
  ...
  methods: {
    ...
    handleDetailDialogOpen (row) {
      ...
      this.$refs.unionDetailDialog.openDialog()
    },
