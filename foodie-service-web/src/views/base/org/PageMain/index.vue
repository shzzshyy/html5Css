    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button type="text" @click="handleDel(scope.row.orgId)" :disabled="!scope.row.hasChildren">删除</el-button>
          <el-divider direction="vertical"></el-divider>
          <el-button type="text" @click="handleAddDialogOpen(scope.row)">添加子级机构</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    handleAddDialogOpen (row) {
      this.dialogForm = {
        parentOrgName: row ? row.orgName : '',
        parentOrgId: row ? row.orgId : '0'
      }
      this.editType = 0
    ...
    handleDelBatch () {
      ...
      const ids = this.multipleSelection.map(item => item.orgId).toString()
      this.$confirm('确认删除选中记录吗？', '提示', {
        type: 'warning'
      }).then(() => {
        this.$api.org.deleteBatch({ ids: ids })
          .then(res => {
            this.$notify({
              title: '删除' + this.bizConfig.bizName + '成功'
            })
