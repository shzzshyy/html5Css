...
export default {
  ...
  computed: {
    ...
    ...mapGetters('d2admin', {
      ...
      initMenu: 'menu/menuGet',
      asideArray: 'menu/asideArrayGet',
      asideOnly: 'menu/asideOnlyGet'
    }),
    /**
  ...
  created () {
    // 初始化菜单搜索功能（仅初始化页面里写死的功能）
    this.$store.commit('d2admin/search/init', this.initMenu)
    // 添加后台的菜单
    this.$store.dispatch('d2admin/account/loadMenus')
  },
  mounted () {
  },
  watch: {
    ...
    '$route.matched': {
      handler (matched) {
        ...
        if (matched.length > 0) {
          const _side = this.asideArray.filter(menu => menu.path === matched[0].path)
          this.$store.commit('d2admin/menu/asideSet', _side.length > 0 ? _side[0].children : [])
        }
      },
      immediate: true
    },
    // F5刷新时会导致动态菜单左侧菜单加载延迟
    asideArray: {
      handler (arrayVal) {
        if (this.asideOnly) {
          return
        }
        const _side = this.asideArray.filter(menu => menu.path === this.$route.matched[0].path)
        this.$store.commit('d2admin/menu/asideSet', _side.length > 0 ? _side[0].children : [])
      },
      immediate: true
    }
